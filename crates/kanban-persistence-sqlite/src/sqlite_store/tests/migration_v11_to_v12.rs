...
async fn seed_v11_with_graph(path: &Path) {
    {
        ...
        sqlx::raw_sql(
            "INSERT INTO boards (id, name, card_prefix, created_at, updated_at)
                 VALUES ('b1','Board','KAN','2024-01-01T00:00:00Z','2024-01-01T00:00:00Z');
             INSERT INTO columns (id, board_id, name, position, wip_limit, created_at, updated_at)
                 VALUES ('c1','b1','Todo',0,7,'2024-01-01T00:00:00Z','2024-01-01T00:00:00Z'),
                        ('c2','b1','Done',1,NULL,'2024-01-01T00:00:00Z','2024-01-01T00:00:00Z');
             INSERT INTO sprints (id, board_id, sprint_number, status, created_at, updated_at)
                 VALUES ('s1','b1',3,'Planning','2024-01-01T00:00:00Z','2024-01-01T00:00:00Z');
             INSERT INTO cards (id, column_id, board_id, title, position, priority, status,
                                card_number, prefix, sprint_id, created_at, updated_at)
                 VALUES ('k1','c1','b1','One',0,'medium','todo',1,'KAN','s1',
                         '2024-01-01T00:00:00Z','2024-01-01T00:00:00Z'),
                        ('k2','c1','b1','Two',1,'medium','todo',2,'KAN',NULL,
                         '2024-01-01T00:00:00Z','2024-01-01T00:00:00Z'),
                        ('k3','c2','b1','Three',0,'medium','todo',3,'KAN',NULL,
                         '2024-01-01T00:00:00Z','2024-01-01T00:00:00Z');
             INSERT INTO spawns_edges (source_id, target_id, created_at)
                 VALUES ('k1','k2','2024-01-01T00:00:00Z');
    ...
    sqlx::raw_sql(
        "CREATE TABLE IF NOT EXISTS board_sprint_counters (
         ...
         INSERT INTO board_sprint_counters (board_id, prefix, counter) VALUES ('b1','KAN',4);
...
fn test_migrate_v11_to_v12_preserves_the_whole_board_subtree() {
    ...
    rt.block_on(async {
        ...
        assert_eq!(
            count(pool, "SELECT COUNT(*) FROM columns WHERE id='c1' AND wip_limit=7").await,
            1,
            "the column's WIP limit was lost"
        );
        assert_eq!(
            count(pool, "SELECT COUNT(*) FROM cards WHERE id='k1' AND sprint_id='s1' AND card_number=1 AND prefix='KAN'").await,
            1,
            "the card's sprint binding, number or prefix was lost"
        );
        assert_eq!(
            count(pool, "SELECT COUNT(*) FROM boards WHERE id='b1' AND card_prefix='KAN' AND name='Board'").await,
            1,

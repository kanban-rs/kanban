...
impl Migrator {
    ...
    pub async fn migrate(
        ...
        match (from, to) {
            ...
            (_, FormatVersion::V18) if from < FormatVersion::V18 => {
                ...
                // v6→v7 onward through v17→v18), not just the destructive tail.
                let backup_path = super::pre_latest_backup_path_for(from, path);
                if let Some(backup) = &backup_path {
                    tokio::fs::copy(path, backup).await?;
                    tracing::info!("Created pre-latest backup at {}", backup.display());
                }
                ...
                match (result, backup_path) {
                    ...
                    (Err(e), Some(backup)) => {
                        tracing::error!(
                            "Migration to V18 failed: {}. Backup preserved at {}",
                            e,
                            backup.display()
                        );
                        Err(e)
    ...
    /// then the v13→v14 default_status derivation, the v14→v15 prefixes
    /// backfill, the v15→v16 card-prefix stamp, the v16→v17 legacy-counter
    /// drop, and the v17→v18 prefix-row repair. Every step is a no-op when
    /// it doesn't apply (each transform short-circuits on a file already at
    /// or beyond its target version), so this is safe to call for any
    /// `from < V18` and always leaves the file at V18.
    async fn run_split_and_upgrade_chain(
        ...
        super::v17_drop_legacy_counters::migrate_v16_to_v17(path).await?;
        super::v18_prefix_rows::migrate_v17_to_v18(path).await
    }
...
mod tests {
    ...
    fn test_detect_version_from_value_rejects_u32_overflow_that_truncates_to_valid_version() {
        ...
        assert!(
            matches!(
                err,
                PersistenceError::UnsupportedFutureVersion { binary_max: 18, .. }
            ),
            "expected UnsupportedFutureVersion, got: {err:?}"
        );
    }

use crate::commands::Command;
use chrono::{DateTime, Utc};
use kanban_core::{AppType, ClientId, KANBAN_VERSION};
use uuid::Uuid;

/// A command with its execution context for audit and distributed tracing.
#[derive(Debug, Clone, serde::Serialize, serde::Deserialize)]
pub struct CommandEnvelope {
    pub command: Command,
    pub correlation_id: Uuid,
    pub issued_by: ClientId,
    pub timestamp: DateTime<Utc>,
    /// Which application surface issued this command.
    pub app_type: AppType,
    /// Kanban version string that produced this command, e.g. "0.7.2".
    pub app_version: String,
    /// Stable per-process lifetime ID; groups commands from one session.
    pub session_id: Uuid,
}

impl CommandEnvelope {
    pub fn new(
        command: Command,
        correlation_id: Uuid,
        issued_by: ClientId,
        detected_at: DateTime<Utc>,
        app_type: AppType,
        app_version: String,
        session_id: Uuid,
    ) -> Self {
        Self {
            command,
            correlation_id,
            issued_by,
            timestamp: detected_at,
            app_type,
            app_version,
            session_id,
        }
    }

    /// Wrap a command with a generated correlation ID and the given client.
    pub fn wrap(command: Command, issued_by: ClientId) -> Self {
        Self::new(
            command,
            Uuid::new_v4(),
            issued_by,
            Utc::now(),
            AppType::Unknown,
            KANBAN_VERSION.to_string(),
            Uuid::new_v4(),
        )
    }
}

/// Wraps a command with a generated correlation ID and `ClientId::nil()` as the issuer.
/// Use [`CommandEnvelope::wrap`] when a real client identity is available.
/// Use `KanbanContext::execute()` to get properly attributed envelopes.
impl From<Command> for CommandEnvelope {
    fn from(command: Command) -> Self {
        Self::wrap(command, ClientId::nil())
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::commands::{BoardCommand, Command, CreateBoard};
    use chrono::Utc;
    use uuid::Uuid;

    fn make_cmd() -> Command {
        Command::Board(BoardCommand::Create(CreateBoard {
            id: Uuid::new_v4(),
            name: "Test".into(),
            card_prefix: None,
            position: 0,
        }))
    }

    #[test]
    fn test_from_command_uses_nil_client_id() {
        let envelope = CommandEnvelope::from(make_cmd());
        assert_eq!(envelope.issued_by, ClientId::nil());
    }

    #[test]
    fn test_from_command_uses_unknown_app_type() {
        let envelope = CommandEnvelope::from(make_cmd());
        assert_eq!(envelope.app_type, AppType::Unknown);
    }

    #[test]
    fn test_from_command_sets_kanban_version() {
        let envelope = CommandEnvelope::from(make_cmd());
        assert_eq!(envelope.app_version, KANBAN_VERSION);
    }

    #[test]
    fn test_from_command_generates_non_nil_session_id() {
        let envelope = CommandEnvelope::from(make_cmd());
        assert_ne!(envelope.session_id, Uuid::nil());
    }

    #[test]
    fn test_wrap_uses_provided_client_id() {
        let client_id = ClientId::new();
        let envelope = CommandEnvelope::wrap(make_cmd(), client_id);
        assert_eq!(envelope.issued_by, client_id);
    }

    #[test]
    fn test_wrap_generates_unique_correlation_id_per_call() {
        let cmd = make_cmd();
        let e1 = CommandEnvelope::wrap(cmd.clone(), ClientId::nil());
        let e2 = CommandEnvelope::wrap(cmd, ClientId::nil());
        assert_ne!(
            e1.correlation_id, e2.correlation_id,
            "each wrap() call must generate a distinct correlation_id"
        );
    }

    #[test]
    fn test_timestamp_is_set_on_construction() {
        let before = Utc::now();
        let envelope = CommandEnvelope::from(make_cmd());
        let after = Utc::now();
        assert!(
            envelope.timestamp >= before && envelope.timestamp <= after,
            "timestamp {} must be within [{}, {}]",
            envelope.timestamp,
            before,
            after
        );
    }

    #[test]
    fn test_new_sets_all_fields() {
        let cmd = make_cmd();
        let corr = Uuid::new_v4();
        let client = ClientId::new();
        let now = Utc::now();
        let session = Uuid::new_v4();
        let envelope = CommandEnvelope::new(
            cmd,
            corr,
            client,
            now,
            AppType::Mcp,
            "1.2.3".into(),
            session,
        );
        assert_eq!(envelope.correlation_id, corr);
        assert_eq!(envelope.issued_by, client);
        assert_eq!(envelope.timestamp, now);
        assert_eq!(envelope.app_type, AppType::Mcp);
        assert_eq!(envelope.app_version, "1.2.3");
        assert_eq!(envelope.session_id, session);
    }

    #[test]
    fn test_serde_round_trip() {
        let client_id = ClientId::new();
        let original = CommandEnvelope::wrap(make_cmd(), client_id);
        let json = serde_json::to_string(&original).unwrap();
        let deserialized: CommandEnvelope = serde_json::from_str(&json).unwrap();
        assert_eq!(deserialized.correlation_id, original.correlation_id);
        assert_eq!(deserialized.issued_by, original.issued_by);
        assert_eq!(deserialized.timestamp, original.timestamp);
        assert_eq!(deserialized.app_type, original.app_type);
        assert_eq!(deserialized.app_version, original.app_version);
        assert_eq!(deserialized.session_id, original.session_id);
    }
}

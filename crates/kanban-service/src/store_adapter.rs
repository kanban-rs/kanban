...
/// so they come last.
///
/// The dependency graph is merged edge-wise rather than replaced: edges
/// already on `store` are kept, an incoming edge between endpoints that
/// already have one is dropped in favour of the existing copy, and an
/// incoming edge that would create a cycle against the existing graph is an
/// error, failing the whole write. When `store`'s graph is empty the merge is
/// skipped and `snapshot.graph` is written verbatim, so its edges keep their
/// original timestamps rather than being regenerated by the merge's setters.
pub fn write_full_snapshot(store: &dyn DataStore, snapshot: Snapshot) -> KanbanResult<()> {
    ...
    let mut graph = store.get_graph()?;
    if graph.is_empty() {
        return store.set_graph(snapshot.graph);
    }
    graph.merge_from(&snapshot.graph)?;

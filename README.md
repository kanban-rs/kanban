...
- Configurable completion columns per board: `board update <board> --completion-columns Done,Decision` (names or UUIDs, most-primary first) declares which columns mean "done" — setting a card's status to done files it under the first entry, moving a card into any of them marks it done, and an empty value (`--completion-columns ''`) turns the status/column coupling off entirely

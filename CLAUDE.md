...
- Envelope: `{ version, metadata, data }`, current version V18; reader accepts V1..V18
- Migration chain V1 → V2 → V3 → (V4/V5 are shape-stable bumps) → V6 (split-graph) → V7 (spawns-bucket rename) → V8 (archived-card board_id backfill) → V9 (archived-board-capable marker) → V10 (archival wrapper collapsed to a pure reference marker) → V11 (historical `cards.board_id` backfill) → V12 (`completion_column_id` replaced by durable `completion_column_ids`, backfilled from the legacy id when it still names a live column, otherwise from the board's last column by `position`/`created_at`/`id`) → … → V15 (`prefixes` backfill, seeded from the legacy per-board counters) → V16 (cards stamped with their addressing prefix) → V17 (per-board `card_counter`/`sprint_counters` dropped, the `prefixes` rows being the sole source of numbering) → V18 (every namespace a card names gets a `prefixes` row, an existing row raised to cover its cards rather than replaced, and any card still carrying no prefix stamped with the one it is addressed by); a pre-chain `.v{N}.backup` is taken before the destructive steps and removed once the migration verifies, so it survives only a failure
...
- Relational schema, 14 tables: metadata, boards, board_sprint_names, columns, sprints, cards, sprint_logs, archived_cards, spawns_edges, blocks_edges, relates_edges, board_archival, command_log, prefixes
- `SUPPORTED_SCHEMA_VERSION = 13` (v13 gives `cards.prefix` a real foreign key to `prefixes(name)`, `ON DELETE RESTRICT ON UPDATE RESTRICT`, carried on a virtual generated column `NULLIF(prefix, '')` so the empty prefix -- exempt from the domain rule this backstops -- stays writable. The v12→v13 rebuild of `cards` runs with `PRAGMA foreign_keys = OFF` (a `DROP TABLE cards` with enforcement on would cascade-empty `sprint_logs`/`archived_cards`), replays the table's index DDL captured from `sqlite_master` rather than a hardcoded list, and verifies the copy against the constraint before the destructive swap -- a database that cannot satisfy it is refused with a typed `PrefixNotBacked` and the original table untouched. v12 drops `boards.card_counter` and the `board_sprint_counters` table; numbering lives on the `prefixes` rows, which the v9→v10 step seeds from those counters before v11→v12 removes them, so the order of the two is load-bearing. The boards rebuild runs with `PRAGMA foreign_keys = OFF`, since every child table cascades on `boards(id)`) (active migrations upgrade older databases on open, each guarded by a durable `VACUUM INTO` pre-migration `.v{N}.backup`); legacy-table drops on open for pre-KAN-405 `command_log`, the retired `undo_state`, and the pre-KAN-504 single `card_edges` table

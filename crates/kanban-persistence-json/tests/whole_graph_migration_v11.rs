//! Whole-graph coverage for the FULL V11 -> V18 chain -- the upgrade every
//! existing v0.8.1 JSON file (whose binary's MAX was V11) actually runs on
//! first open under this release.
//!
//! `tests/whole_graph_migration.rs` proves the V14 -> V18 tail over a
//! non-trivial graph, but the V11-rooted tests (`v11_migration.rs`,
//! `v14_migration.rs`) fixture bare boards with empty `cards`, `sprints`,
//! and edge arrays, so the V11 -> V14 completion/default_status rewrites
//! were never proven to preserve a populated graph. This seeds the same
//! non-trivial shape -- two columns (one WIP-limited), five cards, a sprint
//! with two cards bound, an archived-card marker, and one edge of each kind
//! -- at V11 and asserts everything survives to V18, including the
//! prefix-row seeding and card-prefix stamping that the tail performs.
...
fn v11_whole_graph_fixture() -> Value {
    json!({
        "version": 11,
        "metadata": {
        ...
        "data": {
            "boards": [{
                ...
                "task_list_view": "Flat", "completion_column_id": null, "position": 0,
...
fn write_fixture(path: &std::path::Path) {
    std::fs::write(
        path,
        serde_json::to_string_pretty(&v11_whole_graph_fixture()).unwrap(),
    )
    .unwrap();
}
...
#[tokio::test]
async fn test_migrate_v11_to_v18_preserves_the_whole_entity_graph() {
    ...
    Migrator::migrate(FormatVersion::V11, FormatVersion::MAX, &path)
        .await
        .expect("V11 -> V18 must succeed over a non-trivial graph");
    ...
    assert_eq!(
        todo["default_status"], "Todo",
        "a non-completion column derives to Todo"
    );
    let doing = columns.iter().find(|c| c["id"] == COL_DOING).unwrap();
    assert_eq!(doing["wip_limit"], 3, "the WIP limit on Doing survives");
    assert_eq!(
        doing["default_status"], "Done",
        "the backfilled last-column completion column derives to Done"
    );
    ...
    let before = v11_whole_graph_fixture();
    for id in [CARD_A, CARD_B, CARD_C, CARD_D, CARD_ARCHIVED] {
        assert_eq!(
            card_by_id(&after, id)["card_number"],
            card_by_id(&before, id)["card_number"],
            "card {id} keeps its card_number"
        );
    }
    ...
    assert!(
        ...
        "the board's namespace was seeded into prefix rows: {prefixes:#?}"
    );
    for id in [CARD_A, CARD_B, CARD_C, CARD_D, CARD_ARCHIVED] {
        assert_eq!(
...
#[tokio::test]
async fn test_sync_and_async_chains_agree_on_the_whole_graph_from_v11() {
    let dir = tempdir().unwrap();
    ...
    Migrator::migrate(FormatVersion::V11, FormatVersion::MAX, &async_path)
        .await
        .expect("async V11 -> V18 must succeed");

...
fn test_migrate_v2_db_adds_board_id_and_backfills() {
    ...
    rt.block_on(async {
        ...
        assert_eq!(
            version, 9,
            "schema_version bumped to current (2->3 archived_cards, 4->5 cards.board_id, 5->6 completion columns, 6->7 column default_status, 7->8 default_status derivation, 8->9 completion columns table dropped)"
        );
    });
}

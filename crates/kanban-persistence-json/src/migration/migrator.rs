...
impl Migrator {
    ...
    pub async fn migrate(
        ...
        match (from, to) {
            ...
            (_, FormatVersion::V15) if from < FormatVersion::V15 => {
                ...
                match (result, backup_path) {
                    (Ok(()), Some(backup)) => {
                        if let Err(e) = tokio::fs::remove_file(&backup).await {
                            ...
                            tracing::info!("Migration to V15 verified, backup removed");
                        }
                        Ok(())
                    }
                    (Ok(()), None) => Ok(()),
                    (Err(e), Some(backup)) => {
                        tracing::error!(
                            "Migration to V15 failed: {}. Backup preserved at {}",
                            e,
                            backup.display()
                        );
                        Err(e)
    ...
    /// for any `from < V14` and always leaves the file at V14.
